...
CREATE TABLE IF NOT EXISTS 
  HASHMAP_DB.AWS.IAM_POLICY_MONITORING_LANDING_ZONE(
...
CREATE STAGE IF NOT EXISTS
  HASHMAP_DB.AWS.IAM_POLICY_MONITORING_STAGE
  URL='s3://sf-snowalert-trail/iam_monitoring/policies'
...
// confirm stage works
LIST @HASHMAP_DB.AWS.IAM_POLICY_MONITORING_STAGE;

// create pipe
CREATE OR REPLACE PIPE 
  HASHMAP_DB.AWS.IAM_POLICY_MONITORING_PIPE
  AUTO_INGEST=TRUE
AS 
  COPY INTO 
    HASHMAP_DB.AWS.IAM_POLICY_MONITORING_LANDING_ZONE 
  FROM (
    SELECT 
      ...
      $1 AS POLICY_DATA 
    FROM @HASHMAP_DB.AWS.IAM_POLICY_MONITORING_STAGE
  );
    
// refresh pipe to catch any existing files in the stage
ALTER PIPE HASHMAP_DB.AWS.IAM_POLICY_MONITORING_PIPE REFRESH;

// NOTE: do not forget to add the sqs arn to your s3 bucket for auto_ingest support
SELECT SYSTEM$PIPE_STATUS('HASHMAP_DB.AWS.IAM_POLICY_MONITORING_PIPE');